...
$bloatwareRemovalEnd = $logFile | Where-Object{$_ -match "==Bloatware Removal End=="}
if($config.BloatwareRemoval -and -not $bloatwareRemovalEnd){
    ...
    if($null -eq $exceptions){
        "ERROR`tUnsupported Manufacturer: $manufacturer", "==Bloatware Removal End==" | Add-LogMessage $logPath
    }else{
